...
impl QuantumHunter {
    pub fn new(width: u8, height: u8, ship_lengths: &[u8]) -> Self {
        Self {
            all_possible_layouts: Vec::new(), // Puste na start
    ...
    pub fn new_empty(width: u8, height: u8) -> Self {
        Self {
            ...
            known_board: vec![vec![CellState::Empty; width as usize]; height as usize],
            remaining_ship_lengths: Vec::new(),
            width,
            height,
        }
    }
    ...
    pub fn initialize(&mut self) {
        self.all_possible_layouts = (0..HYPOTHESIS_TARGET)
            .filter_map(|_| hypothesis::generate_random_valid_layout(self.width, self.height, &self.remaining_ship_lengths))
            .collect();
    }
