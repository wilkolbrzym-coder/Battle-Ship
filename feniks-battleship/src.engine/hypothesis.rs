    ...
    for &ship_length in ship_lengths {
        ...
        while !placed && attempts < 100 { // Limit prób, aby uniknąć nieskończonej pętli
            ...
            if can_place_ship(&board, ship_length, Position { x, y }, is_horizontal) {
                for i in 0..ship_length {
                    if is_horizontal {
                        // Używamy Sunk jako placeholdera dla "pola zajętego przez statek"
                        board[y as usize][(x + i) as usize] = CellState::Sunk;
                    } else {
...
mod tests {
    ...
    fn test_generate_layout_succeeds() {
        ...
        // Sprawdźmy, czy plansza zawiera odpowiednią liczbę pól "statków"
        let ship_tiles: usize = layout.unwrap().iter().flatten().filter(|&&cell| cell == CellState::Sunk).count();
        assert_eq!(ship_tiles, ship_lengths.iter().map(|&x| x as usize).sum());
    }
}
